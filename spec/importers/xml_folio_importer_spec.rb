...
RSpec.describe XmlFolioImporter do
  ...
  it "stores data in correct fields in the Folio" do
     ...
     expect(imported_folio.date_created).to include('DateType: start; Day: 08-08-1824 A.D.')
     expect(imported_folio.subject).to include ('Correspondence')
     expect(imported_folio.language).to include ('English')
     expect(imported_folio.identifier).to include ('MS2327-64')
     ...
     expect(imported_folio.copyright_holder).to include ('Public domain')
     ...
     expect(imported_folio.support).to include('ink')
     expect(imported_folio.medium).to include('paper (fiber product)')
