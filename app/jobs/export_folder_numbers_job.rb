class ExportFolderNumbersJob < ApplicationJob
  ...
  def perform(folder_numbers)
    ...
    folder_numbers.each do | folder |
      records << folder.project_id.to_s + "|" + (folder.root_filename || "") + "|" + (folder.title || "") + "|" + (folder.job_type || "") + "|" + (folder.suitable_for_ingest || "") + "|" + (folder.project_name || "") + "|" + (folder.status || "") + "|" + (folder.note || "") + "|" + (folder.created_by || "") + "\n"
    end
    #byebug
